...
(function () {
            ...
            Java.enumerateLoadedClassesSync().forEach(function (className) {
                if (inputClass && inputClass.length > 0) {
                    if (
                        className
                            .toUpperCase()
                            .indexOf(inputClass.toUpperCase()) < 0
                    ) {
                        return;
                    }
                }
                    ...
                    for (var i = 0; i < methods.length; i++) {
                        ...
                        if (inputMethod && inputMethod.length > 0) {
                            if (
                                methodName
                                    .toUpperCase()
                                    .indexOf(inputMethod.toUpperCase()) < 0
                            ) {
                                return;
                            }
                        }
        ...
        Process.enumerateModules().forEach(function (module) {
            if (inputClass && inputClass.length > 0) {
                if (
                    module.name
                        .toUpperCase()
                        .indexOf(inputModule.toUpperCase()) < 0
                ) {
                    return;
                }
            }
    ...
    function init_scan_range() {
        ...
        var fgets = new NativeFunction(addr, "pointer", [
            "pointer",
            "int",
            "pointer",
        ]);
        ...
        var pfile = popen(
            Memory.allocUtf8String(command),
        ...
        while (fgets(buffer, buffer_length, pfile) > 0) {
            ...
            result.push([
                ptr(parseInt(str.substr(0, 8), 16)),
                ptr(parseInt(str.substr(9, 8), 16)),
            ]);
        }
        pclose(pfile);
        return result;
    ...
    function new_scan_by_addr(addr_start, addr_end, bak, input, byte_length) {
        ...
        if (pattern == "error") {
            var data =
                "ERROR:The byte_length can only be 1, 2, 4, 8 and undefined?";
        ...
        var data =
            "new_scan_by_protect scanSync " +
            _addr_start +
            " " +
            (_addr_end - _addr_start) +
            " " +
            pattern;
        klogData(data, "scanlog", data);
        var searchResult_list = Memory.scanSync(
            _addr_start,
            _addr_end - _addr_start,
            pattern
        );
    ...
    function new_scan_by_protect(protection, bak, input, byte_length) {
        ...
        for (var index in range_list) {
            try {
                var data =
                    "new_scan_by_protect scanSync " +
                    range_list[index].base +
                    " " +
                    range_list[index].size +
                    " " +
                    pattern;
                klogData(data, "scanlog", data);
                searchResult_list = Memory.scanSync(
                    range_list[index].base,
                    range_list[index].size,
                    pattern
                );
    ...
    function new_scan_by_addr_unknownValue(
        addr_start,
        addr_end,
        reference,
        byte_length
    ...
    function new_scan_by_addr_littler(
        addr_start,
        addr_end,
        value,
        byte_length
    ...
    function new_scan_by_addr_between(
        addr_start,
        addr_end,
        value1,
        value2,
        ...
        while (true) {
            var op = recv("input", function (data) {
                ...
                if (func == "showMethods") {
                    ...
                    if (mtype == "hexdump") {
                        var res = hexdump(ptr(payload["start"]), {
                            length: payload["size"],
                        });
                    ...
                    if (payload["type"] == "scankey") {
                        next_scan_key(payload["key"]);
                    } else if (payload["type"] == "value") {
                        next_scan_equal(payload["scanvalue"]);
                    }
                }
            });
            op.wait();
    ...
    function checkbreakpoint(pc_addr) {
        switch (arch) {
            case "arm64":
                return (
                    buf2hex(rpc.exports.readdata(pc_addr, 4)) ===
                    breakpoint_desc["breakpoint_ins"]
                );
            case "arm":
                if (check_pc_thumb(pc_addr)) {
                    return (
                        buf2hex(
                            rpc.exports.readdata(
                                pc_addr.and(thumb_erase_maskcode),
                                2
                            )
                        ) === breakpoint_desc["breakpoint_ins"]
                    );
                    ...
                    return (
                        buf2hex(rpc.exports.readdata(pc_addr, 4)) ===
                        breakpoint_desc["thumb_breakpoint_ins"]
                    );
                }
            default:
                klogData(arch + " not support");
    ...
    function resume_pagebreak_write_softbreakpoint(break_info, writer) {
        ...
        if (check_pc_thumb(lr_addr)) {
            ins_writer = new breakpoint_desc["thumb_writer"](
                pc_addr.and(thumb_erase_maskcode)
            );
        }
        ...
        send_dict["ins_content"] = buf2hex(
            rpc.exports.readdata(pc_addr.and(thumb_erase_maskcode), store_size)
        );
        ...
        if (!checkbreakpoint(pc_addr)) {
            Memory.patchCode(pc_addr, store_size, function (code) {
                ...
                switch (arch) {
                    case "arm64":
                        ins_writer.putBytes(
                            hex2buf(breakpoint_desc["breakpoint_ins"])
                        );
                        ins_writer.flush();
                        break;
                    case "arm":
                        if (check_pc_thumb(pc_addr)) {
                            //thumb
                            ins_writer.putBytes(
                                hex2buf(breakpoint_desc["thumb_breakpoint_ins"])
                            );
                            ins_writer.flush();
                        } else {
                            ins_writer.putBytes(
     ...
     * @returns {boolean}
     */
    function resume_pagebreak_write_softbreakpoint_and_show(
        break_info,
        writer,
        details
    ) {
        ...
        let lr_addr = ptr(break_info["current_lr"]);
        const data = buf2hex(
            rpc.exports.readdata(data_addr, break_info["break_len"])
        );
    ...
    function handle_cmd(info, details) {
        ...
        switch (cmd) {
            case 1:
                return resume_pagebreak_write_softbreakpoint(
                    info,
                    breakpoint_desc["writer"]
                );
            case 2:
                return resume_softbreakpoint_set_pagebreak(
                    info,
                    breakpoint_desc["writer"]
                );
            case 3:
                return resume_pagebreak_write_softbreakpoint_and_show(
                    info,
                    breakpoint_desc["writer"],
                    details
                );
